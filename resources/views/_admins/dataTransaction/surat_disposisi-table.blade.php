@extends('layouts.mst')
@section('title', 'Halaman Admin: Tabel Surat Disposisi | '.env('APP_NAME').' - Universitas Cahaya Bangsa')
@push("styles")
    <style>
        td ul, td ol {
            margin: 0 -2em;
    ...
    <div class="right_col" role="main">
        <div class="row">
            <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
                    <div class="x_title">
                        <h2 id="panel_title">Surat Disposisi
                            <small>Table</small>
                        </h2>
                    ...
                    <div id="content1" class="x_content">
                                ...
                                <select id="surat_masuk" class="form-control selectpicker"
                                    ...
                                    @foreach($masuks as $row)
                                        <option value="{{$row->no_surat}}"><strong>{{$row->no_surat}}</strong>&nbsp;&mdash;
                                            {{$row->nama_pengirim.' / '.$row->nama_instansi.', '.$row->asal_instansi}}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <table id="myDataTable" class="table table-striped table-bordered bulk_action">
                            ...
                            @foreach($disposisis as $disposisi)
                                ...
                                <tr>
                                    ...
                                    <td style="vertical-align: middle">{{$disposisi->id}}</td>
                                    <td style="vertical-align: middle">
                                        <strong>{{$masuk->no_surat}}</strong><br>
                                        {{\Carbon\Carbon::parse($masuk->tgl_surat)->format('j F Y')}}
                                    </td>
                                    <td style="vertical-align: middle">
                                        <table>
                                            <tr>
                                            ...
                                            <tr>
                                                ...
                                                <td>&nbsp;:&nbsp;</td>
                                                <td>{{$disposisi->harapan}}</td>
                                            </tr>
                                            <tr>
                                                <td><i class="fa fa-clipboard-list"></i>&nbsp;</td>
                                                <td>Catatan</td>
                                                <td>&nbsp;:&nbsp;</td>
                                                <td>{{$disposisi->catatan != "" ? $disposisi->catatan : '(kosong)'}}</td>
                                            </tr>
                                        </table>
                                    </td>
                        ...
                        <div class="row form-group">
                            ...
                            <form method="post" id="form-sm">
                                {{csrf_field()}}
                                <input id="sd_ids" type="hidden" name="sd_ids">
                            </form>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        $(function () {
                    ...
                    swal({
                        ...
                        preConfirm: function () {
                            return new Promise(function (resolve) {
                                $.get("{{route('massPDF.surat-disposisi', ['ids' => ''])}}/" + ids, function (data) {
            ...
            $('#btn_remove_app').on("click", function () {
                ...
                $("#sd_ids").val(ids);
                $("#form-sm").attr("action", "{{route('massDelete.surat-disposisi')}}");

                if (ids.length > 0) {
                    swal({
                        title: 'Hapus Surat Disposisi',
